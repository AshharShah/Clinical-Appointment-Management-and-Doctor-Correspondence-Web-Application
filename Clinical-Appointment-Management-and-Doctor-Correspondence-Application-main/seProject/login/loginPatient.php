...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($username_err) && empty($password_err)){
        ...
        $sql = "SELECT userid, username, password FROM patients1 WHERE username = ?";
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){                    
                    ...
                    if(mysqli_stmt_fetch($stmt)){
                        if(password_verify($password, $hashed_password)){
                            ...
                            // Redirect user to patient dashboard page
                            header("location: patientDashboard.php");
                            exit;
                        } else{
	...
	<header>
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            ...
            <div class="form-group">
                ...
                <input type="password" name="password" placeholder = "Enter Password." class="date form-control <?php echo (!empty($password_err)) ? 'is-invalid' : ''; ?>">
                <span class="invalid-feedback"><?php echo $password_err; ?></span>
            </div>
            <div class="form-group">
                <input type="submit" class="submitButton btn btn-primary" value="Login">
            </div>
            <p>Don't have an account? <a class = "signLog btn btn-secondary ml-2" href="registerPatient.php">Sign up here</a></p>
        </form>
  
	</div>

	<div class = "docPatient">
		<p id = "main">PATIENT LOG IN.</p></div>
